...
void handleGetMeasurementResult(AsyncWebServerRequest *request) {
  ...
  float averageValue = 16.0;
  sendPUTRequest(averageValue);
  request->send(200, "text/plain", String(averageValue));
}

void sendMeasurementResult() {
  // Enviar el resultado de la medición
  MQ3.update();
  float averageValue = sumValues / sampleCount;
  sendPUTRequest(averageValue);
  client.stop();
...
void handleStartCountdown(AsyncWebServerRequest *request) {
  ...
  if (measuring) {
    ...
    // Iniciar la lógica relacionada con la cuenta regresiva
    // ...

    request->send(200, "text/plain", "Measurement started");
  } else {
    sendMeasurementResult();

    // Detener la lógica relacionada con la cuenta regresiva
    // ...

    // Enviar el resultado de la medición al cliente
    request->send(200, "text/plain", "Measurement stopped");
  }
}
