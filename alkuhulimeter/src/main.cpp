...
void handleGetMeasurementResult(AsyncWebServerRequest *request) {
  ...
  unsigned long loopDuration = 4550; //ms
  while ((endtime - startTime) <= loopDuration) {
    MQ3.update();
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                var countdownInterval = setInterval(function() {
                    ...
                    if (countdown <= 0) {
                        ...
                        // Move the call to getMeasurementResult here
                        getMeasurementResult();

                        // Después de 5 segundos, resetear la barra y habilitar el botón
                        setTimeout(function() {
...
void setup() {
  ...
  MQ3.setA(0.3934); // valores para obtener alcohol con el sensor MQ3
  MQ3.setB(-1.504);
  initializeMQ();
}
